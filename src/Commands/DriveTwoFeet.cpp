...
void DriveTwoFeet::Initialize() {
	...
	float Gi = 0;
	float Gd = 0;
	//float PIDSource = Robot:: driveTrain;
	//float PIDOutput = Robot:: driveTrain;
	PIDDriveTwoFeet = new PIDController(Gp, Gi, Gd,Robot::driveTrain, Robot::driveTrain);
	// Creates a new pID Controller for the PIDdrive2feet

	PIDDriveTwoFeet -> SetOutputRange( -1, 1);
	...
	PIDDriveTwoFeet -> SetSetpoint(ticks_needed);
}

// Called repeatedly when this Command is scheduled to run
void DriveTwoFeet::Execute() {
	//Robot::driveTrain->drive(0, .25, 0);
	// added pid controller so no need to drive
}

// Make this return true when this Command no longer needs to run execute()
bool DriveTwoFeet::IsFinished() {

	if (PIDDriveTwoFeet -> OnTarget() == true || IsTimedOut()) return true;
	else return false;
