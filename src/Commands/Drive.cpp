...
Drive::Drive() {
	...
	SmartDashboard::PutNumber(DELINEARIZATION_POWER_DASHBOARD_KEY, 5);
	SmartDashboard::PutNumber(JOYSTICK_DEADBAND_DASHBOARD_KEY, .05);
}
...
void Drive::Execute() {
	...
	double joystickDeadband = SmartDashboard::GetNumber(JOYSTICK_DEADBAND_DASHBOARD_KEY);

	x = RobotMath::deadband(x,joystickDeadband);
	y = RobotMath::deadband(y,joystickDeadband);
	z = RobotMath::deadband(z,joystickDeadband);

	double delinearizationPower = SmartDashboard::GetNumber(JOYSTICK_DEADBAND_DASHBOARD_KEY);
